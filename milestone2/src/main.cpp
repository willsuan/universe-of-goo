...
void updateRenderGeometry()
{
    ...
    for (std::vector<Connector*>::iterator it = connectors_.begin(); it != connectors_.end(); ++it)
    {
        ...
        case SimParameters::CT_SPRING:
        {
            ...
            if (params_.strainVisualization && spring.restlen > 1e-12)
            {
                // color by strain: green (relaxed) -> yellow -> red (near snap)
                double dist = (particles_[(*it)->p2].pos - particles_[(*it)->p1].pos).norm();
                double strain = std::abs(dist - spring.restlen) / spring.restlen;
                double t = std::min(1.0, strain / std::max(0.01, params_.maxSpringStrain));
                color << t, 1.0 - t, 0.1;
            }
            else if ((*it)->associatedBendingStencils.empty())
            {
                if (spring.canSnap)
    ...
    for (int i = 0; i < nparticles; i++)
    {
        ...
        else if (params_.velocityVisualization && !particles_[i].inert)
        {
            // color by speed: blue (still) -> white (medium) -> red (fast)
            double speed = particles_[i].vel.norm();
            double t = std::min(1.0, speed / 2.0);
            if (t < 0.5)
            {
                double s = t * 2.0;
                color << 0.1 + 0.9 * s, 0.1 + 0.9 * s, 1.0;
            }
            else
            {
                double s = (t - 0.5) * 2.0;
                color << 1.0, 1.0 - s, 1.0 - s;
            }
        }
...
// Bending damping: damps the rate of change of the angle at each hinge
void processBendingDampingForce(const Eigen::VectorXd& q, const Eigen::VectorXd& qprev, Eigen::VectorXd& F)
{
    double h = params_.timeStep;
    if (h < 1e-14)
        return;
    ...
    for (int i = 0; i < (int)bendingStencils_.size(); i++)
    {
        ...
        double cross = e1[0] * e2[1] - e1[1] * e2[0];
        double dot = e1.dot(e2);
        double thetaCur = 2.0 * atan2(cross, l1 * l2 + dot);

        // same for previous config
        Eigen::Vector2d pi0 = qprev.segment<2>(2 * stencil.p1);
        Eigen::Vector2d pj0 = qprev.segment<2>(2 * stencil.p2);
        Eigen::Vector2d pk0 = qprev.segment<2>(2 * stencil.p3);

        Eigen::Vector2d e10 = pj0 - pi0;
        Eigen::Vector2d e20 = pk0 - pj0;
        double l10 = e10.norm();
        double l20 = e20.norm();
        if (l10 < 1e-12 || l20 < 1e-12)
            continue;

        double cross0 = e10[0] * e20[1] - e10[1] * e20[0];
        double dot0 = e10.dot(e20);
        double thetaPrev = 2.0 * atan2(cross0, l10 * l20 + dot0);

        double thetaDot = (thetaCur - thetaPrev) / h;

        // damping force uses the same gradient directions as bending force
        double coeff = -params_.bendingDampingStiffness * thetaDot;
        Eigen::Vector2d rot1(e1[1], -e1[0]);
        Eigen::Vector2d rot2(e2[1], -e2[0]);
        Eigen::Vector2d Fi = coeff * rot1 / (l1 * l1);
        Eigen::Vector2d Fk = coeff * rot2 / (l2 * l2);
        Eigen::Vector2d Fj = -Fi - Fk;
...
void computeForceAndHessian(const Eigen::VectorXd& q, const Eigen::VectorXd& qprev, Eigen::VectorXd& F, Eigen::SparseMatrix<double>& H)
{
    ...
    if (params_.bendingDampingEnabled && params_.bendingEnabled)
...
void computeEnergy(double& kineticEnergy, double& potentialEnergy)
{
    ...
    for (int i = 0; i < (int)particles_.size(); i++)
    {
        if (particles_[i].fixed)
            continue;
        double mass = getTotalParticleMass(i);
        double speed2 = particles_[i].vel.squaredNorm();
        kineticEnergy += 0.5 * mass * speed2;

        if (params_.gravityEnabled)
            potentialEnergy += -params_.gravityG * mass * particles_[i].pos[1];
    }
    ...
    for (int i = 0; i < (int)connectors_.size(); i++)
    {
        if (connectors_[i]->getType() != SimParameters::CT_SPRING)
            continue;
        Spring& s = *(Spring*)connectors_[i];
        double dist = (particles_[s.p2].pos - particles_[s.p1].pos).norm();
        double stretch = dist - s.restlen;
        potentialEnergy += 0.5 * s.stiffness * stretch * stretch;
    }

    for (int i = 0; i < (int)bendingStencils_.size(); i++)
    {
        const BendingStencil& stencil = bendingStencils_[i];
        Eigen::Vector2d pi = particles_[stencil.p1].pos;
        Eigen::Vector2d pj = particles_[stencil.p2].pos;
        Eigen::Vector2d pk = particles_[stencil.p3].pos;
        Eigen::Vector2d e1 = pj - pi;
        Eigen::Vector2d e2 = pk - pj;
        double l1 = e1.norm();
        double l2 = e2.norm();
        ...
        double dot = e1.dot(e2);
        double theta = 2.0 * atan2(cross, l1 * l2 + dot);
        potentialEnergy += 0.5 * stencil.kb * theta * theta;
    }
}
...
void callback()
{
    ...
    if (ImGui::CollapsingHeader("Creative Extras"))
    {
        ImGui::Checkbox("Strain Visualization", &params_.strainVisualization);
        ImGui::Checkbox("Velocity Coloring", &params_.velocityVisualization);
        ImGui::Checkbox("Bending Damping", &params_.bendingDampingEnabled);
        if (params_.bendingDampingEnabled)
            ImGui::InputDouble("  Bend Damping Coeff", &params_.bendingDampingStiffness);
        ImGui::Checkbox("Energy Readout", &params_.showEnergyReadout);
        if (params_.showEnergyReadout)
        {
            double ke, pe;
            computeEnergy(ke, pe);
            ImGui::Text("Kinetic:    %.4f", ke);
            ImGui::Text("Potential:  %.4f", pe);
            ImGui::Text("Total:      %.4f", ke + pe);
        }
    }
